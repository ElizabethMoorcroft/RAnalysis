...
# Change Directory	#
#####################
Name<-"Temporary,NoBoundaries,StraightMove,Density=5,Speed=6.3,Iterations=369-370,StepLength=3.5,CallHalfwidth=0.785398,CameraHalfwidth=0.785398,Freq=45110"
...
for(i in 369){
	...
	Move<-Movement[which(Movement[,9]==i & Movement[,1]==77),]
	Capt<-Captures[which(Captures[,4]==i),]
	
	Move<-Move[which(Move[,3]>3700 & Move[,3]<3800),]
	Move<-Move[which(Move[,4]>3700 & Move[,4]<3800),]
	#--------------------------------
	# For saving
	...
	# ----------------------------
	# PLots the cameras
	for(j in 1){for(i in 369){
	#--------------------------------
	# the movement for the iteration with max captures
	Move<-Movement[which(Movement[,9]==i & Movement[,1]>75 & Movement[,1]<=80),]
	Capt<-Captures[which(Captures[,4]==i),]
	...
	#	
	#	cam<-Camera[which(Camera$ID == Capt$Time_step[k]),]
	#	points( x=c(cam$X.location,cam$X.location+CallRadius*sin(cam$CentreAngle+cam$HalfWidthAngle))
	#			,y=c(cam$Y.location,cam$Y.location+CallRadius*cos(cam$CentreAngle+cam$HalfWidthAngle))
	#			,col="black"
	#			,type="l")
	...
	for(j in 2:dim(Move)[1]){
		...
		if( Move$Re.enterWorld[j]== 0 &
			Move$AnimalNumber[j] == Move$AnimalNumber[j-1]
				...
				points( x=c(Move$Xlocation[j],Move$Xlocation[j]+CallRadius*sin(Move$Angle+Callhalfwidth))
				,y=c(Move$Ylocation[j],Move$Ylocation[j]+CallRadius*sin(Move$Angle+Callhalfwidth))
				,col="blue"
				,type="l")
