...
fileno.speed<-grep("Speed=0.00012",files)
...
for(i in 1:length(fileno)){
	...
	#Captures.temp<-Captures.temp[which(Captures.temp$SensorID == Camera.ID),which(names(Captures.temp) %in% includedcolumns)]
...
for(cameraid in 0:max(Cameras$ID)){
	...
	EndOfSimResults[[cameraid+1]]<-all.results[[10]]
}